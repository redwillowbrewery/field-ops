...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  let query = supabase.from("tasks").select("id,title,task_type,due_at,status,account:accounts(id,name,town,postcode,brewery_available)").eq("assigned_to", userId).order("due_at", { ascending: true, nullsFirst: false }).limit(100);
  if (view === "completed") query = query.eq("status", "completed"); else query = query.eq("status", "open");
  const { data, error } = await query;
  if (error) throw new Error(error.message);
  const tasks = ((data || []) as TaskRow[]).filter((task) => accountAvailable(task.account));
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-950 md:pb-8">
      ...
      <main className="mx-auto max-w-5xl px-4 py-5 sm:px-6"><p className="mb-3 text-sm text-slate-500">{tasks.length} {view === "completed" ? "completed" : "open"} tasks</p>{tasks.length ? <div className="space-y-3">{tasks.map((task)=><TaskCard key={task.id} task={task} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><p className="font-semibold">{view === "completed" ? "No completed tasks yet" : "You’re all caught up"}</p><p className="mt-1 text-sm text-slate-500">Follow-ups created from visits will appear here.</p></div>}</main>
      <BottomNav active="Tasks" />
    </div>
  );
}
function accountAvailable(relation:TaskAccount){const account=Array.isArray(relation)?relation[0]:relation;return !account||account.brewery_available!==false;}
