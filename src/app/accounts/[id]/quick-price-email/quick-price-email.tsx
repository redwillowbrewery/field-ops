...
export function QuickPriceEmail({accountId,accountName,recipient,rows,initialVariantIds,explicitSelection,sellarUrl,livePriceListPath}:{accountId:string;accountName:string;recipient:string;rows:Row[];initialVariantIds:string[];explicitSelection:boolean;sellarUrl:string;livePriceListPath:string|null}){
 ...
 const [livePath,setLivePath]=useState(livePriceListPath);
 const [linkChoice,setLinkChoice]=useState<"customer"|"generic"|"sellar">(livePriceListPath?"customer":"generic");
 ...
 async function openEmail(){
  ...
  const buildSections=(includeDescriptions:boolean)=>groups.map(([f,label])=>{const items=selected.filter(r=>r.format===f);if(!items.length)return"";const products=items.map(r=>{const description=includeDescriptions?plainText(r.description):"";return [`${r.name}`,`${r.packageType} · ${money(r.price)}`,description].filter(Boolean).join("\n")}).join("\n\n");return `\n${label}\n${"-".repeat(label.length)}\n\n${products}`;}).filter(Boolean).join("\n\n");
  const link=linkChoice==="sellar"?sellarUrl:new URL(linkChoice==="customer"&&livePath?livePath:"/price-list",window.location.origin).href;
  const linkLabel=linkChoice==="sellar"?"For more product information, view our Sellar shop:":linkChoice==="customer"&&livePath?"Your live price list with current availability, images and tasting notes:":"Our live catalogue with standard list prices (your prices are shown above):";
  const footer=`\n\n${linkLabel}\n${link}\n\nPrices shown are your current trade prices and availability is subject to confirmation.\n\nCheers,\nRedWillow Brewery`;
  const detailedBody=intro+buildSections(true)+footer;
  const detailedUrl=mailto(to.trim(),subject,detailedBody);
  const useConcise=detailedUrl.length>8000;
  setEmailNotice(useConcise?"This is a large selection, so the email draft uses a concise list without tasting notes. The PDF price list remains the best option for full detail.":"");
  window.location.href=useConcise?mailto(to.trim(),subject,intro+buildSections(false)+footer):detailedUrl;
 }
 return <div className="space-y-4">
  <PriceListShareControls accountId={accountId} initialPath={livePath} onChange={path=>{setLivePath(path);setLinkChoice(path?"customer":"generic")}}/>
  <label className="block rounded-2xl border border-slate-200 bg-white p-4 text-sm font-semibold">Link included in the email<select value={linkChoice} onChange={e=>setLinkChoice(e.target.value as "customer"|"generic"|"sellar")} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white p-3"><option value="customer" disabled={!livePath}>Customer live price list{!livePath?" — create a link above":""}</option><option value="generic">Generic live list — standard prices</option><option value="sellar">Sellar shop</option></select></label>
  ...
  {emailNotice?<p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800">{emailNotice}</p>:null}
...
function mailto(to:string,subject:string,body:string){return `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`}
