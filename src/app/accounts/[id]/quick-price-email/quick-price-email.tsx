...
type Row={format:"cask"|"keg"|"can";packageType:string;name:string;description:string;price:number};
...
export function QuickPriceEmail({accountId,accountName,recipient,rows,sellarUrl}:{accountId:string;accountName:string;recipient:string;rows:Row[];sellarUrl:string}){
 ...
 async function openEmail(){
  ...
  const sections=groups.filter(([f])=>formats.has(f)).map(([f,label])=>{const items=selected.filter(r=>r.format===f);if(!items.length)return"";const lines=items.map(r=>`${r.name} | ${r.packageType} | ${money(r.price)}${r.description?` | ${r.description}`:""}`);return `\n${label}\nBeer | Package | Price | Description\n${lines.join("\n")}`;}).filter(Boolean).join("\n");
