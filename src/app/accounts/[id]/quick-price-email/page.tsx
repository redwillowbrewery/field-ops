...
type EffectivePrice={product_variant_id:string;list_price:number|string|null;customer_price:number|string|null};
...
export default async function QuickPriceEmailPage({params}:{params:Promise<{id:string}>}){
 ...
 const {data:priceRows,error:priceError}=await db.rpc("customer_effective_price_list",{p_account_id:id});if(priceError)throw priceError;const priceByVariant=new Map(((priceRows||[]) as EffectivePrice[]).map(row=>[row.product_variant_id,row]));
 const rows=[] as {format:"cask"|"keg"|"can";packageType:string;name:string;description:string;price:number}[];
 for(const item of availability.items){if(!["cask","keg","can"].includes(item.broadFormat))continue;const priceRow=priceByVariant.get(item.variantId);const price=Number(priceRow?.customer_price??priceRow?.list_price??0);if(price>0)rows.push({format:item.broadFormat as "cask"|"keg"|"can",packageType:packageSalesLabel(item.package,item.packageType),name:item.productName,description:item.presentation?.description||"",price});}
