...
export default async function QuickPriceEmailPage({params}:{params:Promise<{id:string}>}){
 ...
 if(preference==="one_way_only")prices=prices.filter(r=>{
   const product=available.get(r.product_variant_id);
   return r.broad_format==="can"||isOneWayPackage(r.package_type,product?.containerType);
 });
...
function isOneWayPackage(packageType:string,sellarContainer?:string|null){
 const pkg=packageType.toLowerCase().replace(/\s+/g," ").trim();
 const sellar=String(sellarContainer||"").toLowerCase().replace(/[^a-z0-9]+/g,"");
 const sellarOneWay=sellar==="ecask"||sellar==="ekeg";
 const mappedOneWay=pkg.includes("e-cask")||pkg.includes("e-keg");
 return sellarOneWay&&mappedOneWay;
}
