...
export default async function TodayPage() {
  ...
  const [{ data: dueTasks }, { data: upcomingTasks }, { data: appointments }] = await Promise.all([
    ...
    supabase.from("appointments").select("id,purpose,starts_at,ends_at,status,account:accounts(id,name,town,postcode),contact:contacts(full_name)").eq("assigned_to", userId).eq("status", "planned").gte("starts_at", todayStart.toISOString()).lt("starts_at", tomorrow.toISOString()).order("starts_at", { ascending: true }),
  ]);
