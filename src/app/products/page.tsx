import {searchProducts,selectedProduct,searchPage} from "@/lib/product-search";
...
export default async function Products({searchParams}:{searchParams:Promise<{product?:string;error?:string;saved?:string;q?:string;page?:string;source_q?:string;source_page?:string}>}){
 const q=await searchParams;const db=await createClient();const {data:{user}}=await db.auth.getUser();if(!user)redirect("/login");
 const page=searchPage(q.page),sourcePage=searchPage(q.source_page);
 const [catalogue,role,selected]=await Promise.all([searchProducts(db,q.q||"",page),db.rpc("has_capability",{p_capability:"product_edit"}),selectedProduct(db,q.product)]);
 if(catalogue.error||role.error)throw new Error("Product workspace unavailable");
 const editor=role.data===true;
 const {data:publisher,error:publisherError}=await db.rpc("has_capability",{p_capability:"product_publish"});if(publisherError)throw new Error("Product permissions unavailable");
 const [unmapped,packageNames]=await Promise.all([searchProducts(db,q.source_q||"",sourcePage,true),db.from("packages").select("id,name").order("name")]);
 ...
 const visible=(catalogue.data||[]) as {id:string;name:string;active:boolean}[];
 const sourceOptions=(unmapped.data||[]) as {external_id:string;source_name:string}[];
 const pageHref=(p:number,source=false)=>"/products?"+new URLSearchParams({product:q.product||"",q:q.q||"",page:String(source?page:p),source_q:q.source_q||"",source_page:String(source?p:sourcePage)}).toString();
 ...
 const {data:readiness,error:readinessError}=selected&&w?await db.rpc('product_launch_readiness',{p_product:selected.id}):{data:null,error:null};
 if(readinessError)throw new Error('Launch readiness unavailable');
 const pending=readiness?.filter((t:{required:boolean;complete:boolean})=>t.required&&!t.complete).length||0;
 ...
 <form className="flex flex-wrap items-end gap-3"><input type="hidden" name="product" value={q.product||""}/><label>Find a beer<input name="q" defaultValue={q.q||""} maxLength={200} className={input}/></label><button className="rounded-lg border px-4 py-2">Search beers</button><span>{catalogue.count||0} matches · page {page}</span>{page>1&&<Link className="underline" href={pageHref(page-1)}>Previous</Link>}{page*50<(catalogue.count||0)&&<Link className="underline" href={pageHref(page+1)}>Next</Link>}</form>
 <div className="grid gap-5 sm:grid-cols-2">
 <form className="rounded-xl border p-4"><h2 className="font-semibold">Open a beer</h2><label>Beer<select name="product" className={input} defaultValue={selected?.id||""}><option value="">Choose a beer</option>{selected&&!visible.some(p=>p.id===selected.id)&&<option value={selected.id}>{selected.name}</option>}{visible.map(p=><option value={p.id} key={p.id}>{p.name}{p.active?"":" · inactive / draft"}</option>)}</select></label><button className="mt-3 rounded-lg border px-4 py-2">Open</button></form>
 ...
 {editor&&!sources?.data?.length&&<form className="mt-4 flex flex-wrap items-end gap-3"><input type="hidden" name="product" value={selected.id}/><label>Find a ViewPlan record<input name="source_q" defaultValue={q.source_q||""} maxLength={200} className={input}/></label><button className="rounded-lg border px-3 py-2">Search source records</button><span>{unmapped.count||0} matches · page {sourcePage}</span>{sourcePage>1&&<Link className="underline" href={pageHref(sourcePage-1,true)}>Previous</Link>}{sourcePage*50<(unmapped.count||0)&&<Link className="underline" href={pageHref(sourcePage+1,true)}>Next</Link>}</form>}
 {editor&&!sources?.data?.length&&<form action={linkViewPlan} className="mt-5 rounded-lg border p-3"><Hidden product={selected.id} revision={w.revision}/><label>Link an existing ViewPlan product<select name="external" required className={input}><option value="">Choose an exact source record</option>{sourceOptions.map(s=><option key={s.external_id} value={s.external_id}>{s.source_name} · {s.external_id}</option>)}</select></label><p className="my-2 text-sm">Choose the corresponding product created in ViewPlan. Existing links cannot be replaced or merged here.</p><Submit>Confirm exact source link</Submit></form>}
 ...
 {publisher&&<form action={publishProduct} className="mt-5 border-t pt-4"><Hidden product={selected.id} revision={w.revision}/><p className="mb-2 text-sm">Publish the saved draft. Unsaved changes above are not included.</p><Submit>Publish saved revision</Submit></form>}
 ...
 {readiness&&<ul className="my-3 space-y-2">{readiness.filter((r:{kind:string})=>r.kind!=="external").map((r:{kind:string;title:string;complete:boolean})=><li key={r.kind}>{r.complete?"✓ ":"Pending: "}{r.title} · derived from the current revision</li>)}</ul>}
 {tasks?.data?.filter(t=>t.kind==="external").map(t=><details key={t.id+":"+t.revision} className="my-3 rounded-lg border p-3"><summary className="cursor-pointer font-medium">{t.complete?"✓ ":""}{t.title}{t.required?" · required":""}{t.owner_name?" · "+t.owner_name:""}</summary><form action={saveLaunchTask} className="my-3 grid gap-3 rounded-lg border p-3 sm:grid-cols-2">
