...
export function AppointmentForm({ contacts, initial }: { contacts: Contact[]; initial: Initial }) {
  ...
  return <form action={formAction} className="space-y-5">
    ...
    <button disabled={pending} className="h-12 w-full rounded-xl bg-slate-950 px-4 font-semibold text-white">{pending ? "Saving…" : initial.id ? "Save changes" : "Create appointment"}</button>
  </form>;
}
