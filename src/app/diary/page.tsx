...
type AccountRef={id:string;name:string|null;town:string|null;postcode:string|null;brewery_available?:boolean|null};
...
export default async function DiaryPage({ searchParams }: { searchParams: Promise<{ days?: string }> }) {
  ...
  const { data: appointments, error } = await supabase.from("appointments").select("id,starts_at,ends_at,purpose,status,account:accounts(id,name,town,postcode,brewery_available),contact:contacts(full_name)").eq("assigned_to", authData.user!.id).gte("starts_at", start.toISOString()).lt("starts_at", end.toISOString()).neq("status", "cancelled").order("starts_at", { ascending: true });
  if (error) throw new Error(error.message);
  const grouped = new Map<string, DiaryAppointment[]>();
  for (const appointment of (appointments || []) as DiaryAppointment[]) { if(!accountAvailable(appointment.account)) continue; const key = appointment.starts_at.slice(0,10); grouped.set(key, [...(grouped.get(key) || []), appointment]); }
