...
export function composeComingSoon(rows:PlanningPreview[],prices:Price[],preference:AccountContainerPreference,information:InformationData={products:[],packages:[]}):ComingSoonItem[]{
 ...
 for(const row of rows){
  ...
  if(!item){item={productId:row.productId,productName:row.productName,description:row.description,imageUrl:row.imageUrl,abv:row.abv,estimatedDate:null,packages:[]};result.set(row.productId,item);}
  ...
  item.packages.push({id:row.package.id,label:packageSalesLabel(row.package,row.package.name),broadFormat:row.package.broad_format,price:money(price?.customer_price??price?.list_price),estimatedDate:row.estimatedDate,information:effectiveProductInformation(information,row.productId,row.package.id)});
 }
...
export async function getComingSoon(db:SupabaseClient,accountId:string|null,preference:AccountContainerPreference):Promise<ComingSoonResult>{
 ...
 try{
  ...
  const [prices,information]=await Promise.all([getEffectivePrices(db,accountId,ids),readProductInformation(db,permitted.map(row=>row.productId))]);
  return{status:"fresh",observedAt:data.observedAt,items:composeComingSoon(permitted,prices,preference,information)};
