...
export async function getPublicPriceList(token?: string) {
  ...
  if (token === undefined) return { accountName: null, selling: await getGenericSellingData(db) };
  ...
  const selling = await getAccountSellingData(db, link.account_id, (account.container_preference || "any") as AccountContainerPreference);
  ...
  return stillActive ? { accountName: account.name as string, selling } : null;
}
