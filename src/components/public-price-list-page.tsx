...
import {ComingSoonPriceList} from "@/components/coming-soon-price-list";
...
export async function PublicPriceListPage({ token, format = "all" }: { token?: string; format?: string }) {
  ...
  if (!result) notFound();
  const { accountName, selling, comingSoon } = result;
  ...
  return <div className="min-h-screen bg-slate-50 pb-10 text-slate-950 print:bg-white">
    ...
    <main className="mx-auto max-w-5xl px-4 py-5 sm:px-6"><h2 className="mb-4 text-xl font-semibold">Available now</h2><DecoratedPriceList selling={selling} format={selectedFormat} />
      <ComingSoonPriceList comingSoon={comingSoon} format={selectedFormat}/>
