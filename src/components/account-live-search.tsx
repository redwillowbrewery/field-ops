...
export function AccountLiveSearch({initialValue=""}:{initialValue?:string}){
 const router=useRouter();const pathname=usePathname();const searchParams=useSearchParams();const currentQuery=searchParams.toString();const [value,setValue]=useState(initialValue);
 useEffect(()=>{const timer=setTimeout(()=>{const params=new URLSearchParams(currentQuery);const next=value.trim();const current=(params.get("q")||"").trim();if(next===current)return;if(next)params.set("q",next);else params.delete("q");router.replace(`${pathname}${params.toString()?`?${params.toString()}`:""}`,{scroll:false});},250);return()=>clearTimeout(timer);},[value,pathname,router,currentQuery]);
