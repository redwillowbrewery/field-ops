...
await db.exec('reset role');
await db.exec("alter table packages add column name text,add column broad_format text default 'cask',add column source_system text,add column source_reference text,add column active boolean default true");
await db.exec("insert into packages(id,name,capacity_litres,source_system,source_reference) select gen_random_uuid(),n,20,'viewplan',n from unnest(array['E-Cask','Firkin','Pin','Pin (Flat Bottom)']) n");
await db.exec(readFileSync(new URL('../supabase/migrations/20260908180000_take_off_cask_formats.sql',import.meta.url),'utf8'));
await asUser(sales);
const formats=(await db.query("select id,name from take_off_package_options($1) where take_off_enabled",[product])).rows;
assert.equal(formats.length,4);
for(const format of formats)await db.query("select save_take_off_request($1,$2,2,'2026-09-25','Planning without sales variant')",[s.id,format.id]);
await db.exec('reset role');
assert.equal((await db.query('select count(*)::integer n from product_variants')).rows[0].n,1);
await db.query('update packages set active=false where id=$1',[formats[0].id]);
await asUser(sales);
await assert.rejects(db.query("select save_take_off_request($1,$2,2,'2026-09-25','Inactive rejected')",[s.id,formats[0].id]));
assert.equal((await db.query('select count(*)::integer n from take_off_package_options(null)')).rows[0].n,0);
