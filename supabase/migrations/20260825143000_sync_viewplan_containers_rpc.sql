-- Brewery Ops - server-side ViewPlan container snapshot merge
-- Resolves ViewPlan customer IDs to canonical accounts inside Postgres so the
-- Windows PowerShell connector never needs to download the full account map.
...
comment on function public.sync_viewplan_containers(jsonb) is
  'Atomically replaces the ViewPlan off-site container snapshot after resolving ViewPlan customer IDs server-side and validating mapping coverage.';
