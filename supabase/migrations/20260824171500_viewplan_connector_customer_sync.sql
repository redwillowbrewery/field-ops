-- Brewery Ops - ViewPlan connector state + customer batch sync

create table if not exists public.connector_sync_state (
  source_system text not null,
  module text not null,
  last_source_lud timestamptz,
  last_success_at timestamptz,
  last_full_sync_at timestamptz,
  last_row_count integer not null default 0,
  last_error text,
  updated_at timestamptz not null default now(),
  primary key (source_system, module)
);
...
create table if not exists public.connector_sync_runs (
  ...
  source_from_lud timestamptz,
  source_to_lud timestamptz,
...
create index if not exists connector_sync_runs_module_started_idx
  on public.connector_sync_runs(source_system, module, started_at desc);
...
begin
  ...
  loop
    ...
    insert into public.accounts (
      ...
      do_not_call,
      do_not_email,
      brewery_sales_channel,
      brewery_last_call_date,
      brewery_next_call_date,
      brewery_call_days,
      brewery_call_time,
      brewery_call_schedule,
      brewery_last_synced_at
    ) values (
      source_id,
    ...
    -- ViewPlan owns only numbered BMS contact slots. CRM-only contacts have no slot and are untouched.
...
comment on table public.connector_sync_state is 'High-water marks for read-only source connectors such as ViewPlan.';
comment on table public.connector_sync_runs is 'Audit log for Brewery Ops connector runs.';
comment on function public.sync_viewplan_customers(jsonb) is 'Batch merges ViewPlan-owned customer/contact fields without overwriting CRM-owned relationship state or activity.';
